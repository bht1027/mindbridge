...
| 7 | Ground responses with lightweight support knowledge. | Keyword-overlap retrieval returns top-k support KB entries and records retrieval metadata, including CBT-informed support snippets. | `retriever.py`, `data/support_kb.json`, `pipeline.py` | Debug screenshot showing retrieved hit IDs. |
...
| 10 | Include quantitative metrics and statistical significance. | Evaluation records runtime, response length, LLM-judge scores, and paired bootstrap confidence intervals versus baseline. | `evaluate.py`, `metrics.py`, `judge.py` | Quality summary and 95% CI table from generated report. |
...
| Presentation: evaluation | Supported by code | Requires generated `evaluation_report.md` tables before final submission. |
...
- Generate final `evaluation_results.json` and `evaluation_report.md` using `scripts/run_eval_full.sh`.
